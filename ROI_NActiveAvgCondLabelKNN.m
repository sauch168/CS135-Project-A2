...
%relevantROI = {'CALC', 'LIPL', 'LIPS', 'LOPER'};
...
for n=1:length(starplus_data)
    for j=n+1:length(starplus_data)
        for k=j+1:length(starplus_data)
            ...
            avgKNNClassifier = fitcknn(trainingEx, trainingL);
            predictedLabels = predict(avgKNNClassifier, trainingEx);
            avgKNNaccuracy = (sum(predictedLabels == testingL)) / length(predictedLabels);

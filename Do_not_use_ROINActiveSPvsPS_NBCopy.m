...
for n=1:length(starplus_data)
    ...
    [Xi,Xd,Xm]=transformIDM_selectTrials(current_set.info, ...
        current_set.data, ...
        current_set.meta, ...
        find([current_set.info.cond]>1));
    [Xi,Xd,Xm]=transformIDM_selectTimewindow(Xi,Xd,Xm,[1:32]);
    currentEx=[];
    currentL=[];
    currentI=[];
    [i,d,m]=transformIDM_selectTrials(Xi,Xd,Xm,find([Xi.firstStimulus]=='P'));
    ...
    [i,d,m]=transformIDM_selectTrials(Xi,Xd,Xm,find([Xi.firstStimulus]=='S'));
    for z=1:length(relevantROI)
        [Ri,Rd,Rm]=transformIDM_selectROIVoxels(i,d,m,relevantROI(z));
        [Ri,Rd,Rm]=transformIDM_selectActiveVoxels(Ri,Rd,Rm,numVoxels);
        [Rex,~,Rinfo]=idmToExamples_condLabel(Ri,Rd,Rm);
